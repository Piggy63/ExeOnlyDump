                    /* ExeOnlyDump */
/* Copyright (c) 2023 Porky Piggy. All rights reserved. */
...
/* REVISION HISTORY
   ...
   === WRITTEN & TESTED UNDER UNIVERSITY RHEL SERVER ===
   2023-03-26         Written to dump A1 demo (uses constructors and LD_PRELOAD).
   2023-04-07         Rewritten to support static and SUID binaries (hooks sys_write to dump program to stdout).
   2023-04-09         Modified to hook any syscall to dump program to disk file. Added crude "debugger".

   === MODIFIED & TESTED UNDER UBUNTU WSL ===
   2023-08-12         Cleaned up, fixed pipe full bug and renamed to ExeOnlyDump (xodump).
   2023-08-13         Broken into three C files.
   2023-08-24         More eye candy, macro for error messages.
   2023-08-25  1.00   Added unattended/non-interactive mode and version number.
...
static void dbg_dump(pid_t pid, int *fds)
{
	...
	if (prompt_yes_no())
	{
		...
		{
			off_t size;
			if (ensure_file_exists(output) && get_file_size(output, &size) && set_file_size(output, (size > (offset + length)) ? size : offset + length))
			{
...
int main(int argc, char **argv)
{
	...
	"(c) 2023 Porky Piggy. All rights reserved.\n"
